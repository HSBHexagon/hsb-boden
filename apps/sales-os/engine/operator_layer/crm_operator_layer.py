...
def main():
    ...
    grid = sheets.spreadsheets().get(spreadsheetId=SID, fields="sheets(properties(sheetId,gridProperties(columnCount)),filterViews(title),conditionalFormats(booleanRule(condition(values))))").execute()
    ...
    existing_cfs = sh_obj.get("conditionalFormats", [])
    ...
    # 11) Zeilentönung nach Pipeline: alte =$BE2=-Regeln entfernen (hinten nach vorn), neue anfügen
    stale_idx = [i for i, cf in enumerate(existing_cfs)
                 if cf.get("booleanRule", {}).get("condition", {}).get("values", [{}])[0].get("userEnteredValue", "").startswith("=$BE2=")]
    for i in sorted(stale_idx, reverse=True):
        req.append({"deleteConditionalFormatRule": {"sheetId": SH, "index": i}})
    req += pipeline_row_tints(sheet_id=SH, last_row=N, last_col=57)

    print("Requests:", len(req), "| DRY-RUN" if DRY else "| APPLY")
    if DRY:
        print(json.dumps([list(r.keys())[0] for r in req]))
        print(f"Zeilentönung: {len(stale_idx)} bestehende =$BE2=-Regeln würden gelöscht, 7 neue angefügt.")
        sys.exit(0)
    sheets.spreadsheets().batchUpdate(spreadsheetId=SID, body={"requests":req}).execute()
    # 12) Pipeline-Header + Formel
