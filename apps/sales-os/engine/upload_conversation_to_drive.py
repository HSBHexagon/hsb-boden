...
repo_root = Path("/Users/joelcherinodiaz/KI-System/02_Projects/active/hsb-sales-os")
out_dir = repo_root / "docs" / "handoff"
out_dir.mkdir(parents=True, exist_ok=True)
md_path = out_dir / "HSB_Sales_OS_2000_Drafts_Gesamtes_Gespraech_2026-09-15.md"

conversation_text = """# HSB Sales OS — Gesamtes Gespräch, Audit & Abschluss-Protokoll (2.000 Drafts)
...
print(f"Lokal gespeichert: {md_path}")

# Upload to Google Drive (Folder 1LrvCBlWEAcYFjm4rm6a7YLDwfrHrvxJW)
...
parent_id = "1LrvCBlWEAcYFjm4rm6a7YLDwfrHrvxJW"

# 1. Upload Markdown File
media_md = MediaFileUpload(str(md_path), mimetype="text/markdown", resumable=True)
file_metadata_md = {
    "name": "HSB_Sales_OS_2000_Drafts_Gesamtes_Gespraech_2026-09-15.md",
    "parents": [parent_id]
}
uploaded_md = drive_service.files().create(body=file_metadata_md, media_body=media_md, fields="id, name, webViewLink").execute()
print(f"Markdown hochgeladen: ID {uploaded_md.get('id')} -> {uploaded_md.get('name')}")
print(f"MD Link: {uploaded_md.get('webViewLink')}")

# 2. Upload as Google Doc (converted)
media_gdoc = MediaFileUpload(str(md_path), mimetype="text/plain", resumable=True)
file_metadata_gdoc = {
    "name": "HSB Sales OS – Gesamtes Gespräch & Abschluss-Audit – 2026-09-15",
    "mimeType": "application/vnd.google-apps.document",
    "parents": [parent_id]
}
uploaded_gdoc = drive_service.files().create(body=file_metadata_gdoc, media_body=media_gdoc, fields="id, name, webViewLink").execute()
print(f"Google Doc erstellt: ID {uploaded_gdoc.get('id')} -> {uploaded_gdoc.get('name')}")
print(f"Doc Link: {uploaded_gdoc.get('webViewLink')}")
