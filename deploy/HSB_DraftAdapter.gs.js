...
function createDraftsForBatch(batchId, options) {
  ...
  // readLeads_() nimmt keinen Parameter entgegen und liefert
  // {header, index, rows, leads} — selbst nach Batch filtern.
  ...
  for (var i = 0; i < leads.length && results.attempted < limit; i++) {
    ...
    var rendered = renderEmail_(lead, flyerFelder._flyer);   // Klartext aus der Engine
    var payload = {
      leadId: lead.Lead_ID,
      batchId: batchId,
      owner: ownerKey,
