...
var leadFormSchema = external_exports.object({
  firstName: external_exports.string().min(2),
  lastName: external_exports.string().min(2),
  company: external_exports.string().min(2),
  email: external_exports.string().email(),
  phone: external_exports.string().min(5),
  industry: external_exports.string().min(2),
  projectType: external_exports.string().min(2),
  areaSize: external_exports.string().optional(),
  currentFloor: external_exports.string().optional(),
  loads: external_exports.array(external_exports.string()).min(1),
  liveOperation: external_exports.string().min(2),
  timeframe: external_exports.string().optional(),
  message: external_exports.string().min(10),
  privacyConsent: external_exports.literal(true)
});
...
function sanitizeUtmValue(value) {
  ...
  const cleaned = value.replace(/[^\w .~%+-]/g, "").trim().replace(/^[=+@-]+/, "").trim().slice(0, UTM_MAX);
  return cleaned.length > 0 ? cleaned : void 0;
}
__name(sanitizeUtmValue, "sanitizeUtmValue");
function sanitizePathValue(value) {
  if (typeof value !== "string") return void 0;
  const cleaned = value.replace(/[\u0000-\u001F\u007F<>"'`\\]/g, "").trim().slice(0, PATH_MAX);
  return cleaned.length > 0 ? cleaned : void 0;
}
__name(sanitizePathValue, "sanitizePathValue");
function sanitizePagePath(value) {
  if (typeof value !== "string") return void 0;
  const withoutQuery = value.split(/[?#]/, 1)[0];
  const cleaned = sanitizePathValue(withoutQuery);
  if (!cleaned || !cleaned.startsWith("/")) return void 0;
  return cleaned;
}
__name(sanitizePagePath, "sanitizePagePath");
function sanitizeReferrerOrigin(referrer, ownOrigin = "") {
  if (typeof referrer !== "string" || !referrer) return void 0;
  try {
    const url = new URL(referrer);
    if (ownOrigin && url.origin === ownOrigin) return void 0;
...
__name(sanitizeReferrerOrigin, "sanitizeReferrerOrigin");
function hasCampaign(attr) {
  return UTM_KEYS.some((key) => Boolean(attr[key]));
}
__name(hasCampaign, "hasCampaign");
...
var SITE_ORIGIN = "https://www.hsb-boden.de";
var leadEndpointSchema = external_exports.object({
  firstName: external_exports.string().trim().min(2).max(80),
  lastName: external_exports.string().trim().min(2).max(80),
  company: external_exports.string().trim().min(2).max(120),
  email: external_exports.string().trim().email(),
  phone: external_exports.string().trim().min(5),
  industry: external_exports.string().trim().min(2),
  projectType: external_exports.enum(["neubau", "sanierung", "bewertung"]),
  areaSize: external_exports.string().trim().max(80).optional(),
  liveOperation: external_exports.enum(["ja", "nein", "unklar"]),
  loads: external_exports.array(external_exports.enum(loadOptions)).min(1),
  message: external_exports.string().trim().min(10).max(2e3),
  privacyConsent: external_exports.literal(true),
  source: external_exports.string().trim().min(1),
  legalBasis: external_exports.string().trim().min(1),
  access_key: external_exports.string().trim().optional(),
  ...
  utm_source: external_exports.unknown().optional().transform(sanitizeUtmValue),
  utm_medium: external_exports.unknown().optional().transform(sanitizeUtmValue),
  utm_campaign: external_exports.unknown().optional().transform(sanitizeUtmValue),
  utm_term: external_exports.unknown().optional().transform(sanitizeUtmValue),
  utm_content: external_exports.unknown().optional().transform(sanitizeUtmValue),
  referrer: external_exports.unknown().optional().transform((v) => sanitizeReferrerOrigin(v, SITE_ORIGIN)),
  landing_page: external_exports.unknown().optional().transform(sanitizePagePath),
  form_path: external_exports.unknown().optional().transform(sanitizePagePath),
  attribution_channel: external_exports.unknown().optional().transform(
    (v) => v === "campaign" || v === "referral" || v === "direct" ? v : void 0
  ),
  honeypot: external_exports.string().trim().max(0).optional(),
  timestamp: external_exports.number().optional()
}).transform(({ honeypot, ...rest }) => {
  if (rest.attribution_channel !== void 0 || rest.utm_source !== void 0 || rest.utm_medium !== void 0 || rest.utm_campaign !== void 0 || rest.utm_term !== void 0 || rest.utm_content !== void 0 || rest.referrer !== void 0) {
...
// api/lead.ts
var ALLOWED_ORIGINS = /* @__PURE__ */ new Set(["https://hsb-boden.de", "https://www.hsb-boden.de"]);
var MAX_PAYLOAD_BYTES = 16 * 1024;
var IP_LIMIT = { max: 5, windowMs: 10 * 60 * 1e3 };
var EMAIL_LIMIT = { max: 2, windowMs: 30 * 60 * 1e3 };
var WEBHOOK_TIMEOUT_MS = 6e3;
var MIN_WEBHOOK_TOKEN_LENGTH = 32;
var MAX_WEBHOOK_TOKEN_LENGTH = 512;
var memoryIpHits = /* @__PURE__ */ new Map();
var memoryEmailHits = /* @__PURE__ */ new Map();
function isRateLimitedMemory(key, store, limit, now) {
  const hits = (store.get(key) ?? []).filter((t) => now - t < limit.windowMs);
  hits.push(now);
  store.set(key, hits);
  return hits.length > limit.max;
}
__name(isRateLimitedMemory, "isRateLimitedMemory");
async function isRateLimited(kv, memoryStore, key, limit, now) {
  if (!kv) return isRateLimitedMemory(key, memoryStore, limit, now);
  const raw = await kv.get(key, "json");
  const hits = (raw ?? []).filter((t) => now - t < limit.windowMs);
  hits.push(now);
  await kv.put(key, JSON.stringify(hits), { expirationTtl: Math.max(60, Math.ceil(limit.windowMs / 1e3)) });
  return hits.length > limit.max;
}
...
function isAllowedOrigin(origin) {
  ...
  try {
    ...
    return url.origin === origin && url.protocol === "https:" && /^[a-z0-9-]+\.hsb-boden\.pages\.dev$/.test(url.hostname);
  } catch {
    return false;
  }
}
...
__name(isAllowedAppsScriptUrl, "isAllowedAppsScriptUrl");
function isStrongWebhookToken(value) {
  return typeof value === "string" && value.length >= MIN_WEBHOOK_TOKEN_LENGTH && value.length <= MAX_WEBHOOK_TOKEN_LENGTH && value.trim() === value && !/[\u0000-\u001f\u007f]/.test(value);
}
__name(isStrongWebhookToken, "isStrongWebhookToken");
function parseAuthenticatedWebhookConfig(rawConfig) {
  ...
  if (keys.length !== 2 || !keys.includes("url") || !keys.includes("token") || typeof config.url !== "string" || !isAllowedAppsScriptUrl(config.url) || !isStrongWebhookToken(config.token)) {
    throw new Error("invalid_webhook_config");
  }
  return { url: config.url, token: config.token };
}
__name(parseAuthenticatedWebhookConfig, "parseAuthenticatedWebhookConfig");
function resolveWebhookTarget(env, lead) {
  if (env.LEAD_WEBHOOK_CONFIG !== void 0) {
...
__name(resolveWebhookTarget, "resolveWebhookTarget");
var PayloadTooLargeError = class extends Error {
  static {
    __name(this, "PayloadTooLargeError");
  }
};
async function readBodyWithLimit(request, limitBytes) {
...
var onRequestPut = onRequestGet;
var onRequestDelete = onRequestGet;
var onRequestPost = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    if (target.requireAcknowledgement) {
      const acknowledgement = await webhookResponse.json();
      if (!acknowledgement || typeof acknowledgement !== "object" || Array.isArray(acknowledgement) || Object.keys(acknowledgement).length !== 1 || acknowledgement.ok !== true) {
...
// ../.wrangler/tmp/pages-EytZF9/functionsRoutes-0.2541382414042921.mjs
var routes = [
  {
    routePath: "/api/lead",
    mountPath: "/api",
...
function pages_template_plugin_default(pluginArgs) {
  const onRequest = /* @__PURE__ */ __name(async (workerContext) => {
