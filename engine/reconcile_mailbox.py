...
3. Graph API Sent-Items Abgleich (--sync-sent):
   Liest gesendete Mails aus Outlook (SentItems) und gleicht sie über
   Internet_Message_ID und Empfängeradresse mit ALL_LEADS ab.
4. Graph API Inbound-Antworten Abgleich (--sync-inbound):
   Liest Antworten aus dem Posteingang und setzt Reply_Status bzw. Opt-Outs.
"""
import argparse
import datetime
import json
...
def get_leads_status():
    ...
    # Lese Spalten A (Lead-ID), I (E-Mail), AA (Owner), AO (Send_Status), AP (Send_Datum), AR (Reply_Status), AU (Batch_Status)
    # Zeilen 2 bis 6425
    ranges = ["ALL_LEADS!A2:A6425", "ALL_LEADS!I2:I6425", "ALL_LEADS!AA2:AA6425", 
              "ALL_LEADS!AO2:AO6425", "ALL_LEADS!AP2:AP6425", "ALL_LEADS!AR2:AR6425", "ALL_LEADS!AU2:AU6425"]
...
def main():
    ...
    parser.add_argument("--mark-sent", nargs=2, type=int, metavar=("START_ROW", "END_ROW"), 
    ...
    if args.sync_apple_mail:
        ...
        leads = get_leads_status()
        owner_drafts = [l for l in leads if l["owner"] == args.mark_sent_owner and l["send_status"] == "drafted"]
        if not owner_drafts:
