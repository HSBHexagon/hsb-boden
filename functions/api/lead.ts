...
async function isRateLimited(
  ...
  if (!kv) {
    throw new Error("missing_rate_limit_kv");
  }
...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    if (err instanceof Error && err.message === "missing_rate_limit_kv") {
      console.error(JSON.stringify({ ts: new Date(now).toISOString(), result: "error", code: "missing_rate_limit_kv" }));
      return jsonResponse(500, { ok: false, error: "internal_server_error" }, origin);
    }
    throw err;
  }
