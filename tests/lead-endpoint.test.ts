...
function makeRequest(
  body: unknown,
  opts: { ip?: string; origin?: string; method?: string } = {},
) {
  const {
    ip = "203.0.113.1",
...
describe("POST /api/lead", () => {
  ...
  it("uses an authenticated envelope when LEAD_WEBHOOK_CONFIG is present", async () => {
    ...
    const res = await onRequestPost(
      makeContext(makeRequest(validBody), authenticatedEnv),
    );
    ...
    expect(url).toBe(
      "https://script.google.com/macros/s/AUTHENTICATED_EXAMPLE/exec",
    );
  ...
  it("fails closed instead of falling back when LEAD_WEBHOOK_CONFIG is invalid JSON", async () => {
    ...
    const res = await onRequestPost(
      makeContext(makeRequest(validBody), {
        ...testEnv,
  ...
  it.each([
    [
      "HTTP URL",
      "http://script.google.com/macros/s/AUTHENTICATED_EXAMPLE/exec",
      "test-only-auth-token-with-32-characters",
    ],
    [
      "lookalike host",
      "https://script.google.com.evil.example/macros/s/AUTHENTICATED_EXAMPLE/exec",
      "test-only-auth-token-with-32-characters",
    ],
    [
      "URL fragment",
      "https://script.google.com/macros/s/AUTHENTICATED_EXAMPLE/exec#fragment",
      "test-only-auth-token-with-32-characters",
    ],
    [
      "token whitespace",
  ...
  it("rejects deeply nested JSON to prevent DoS", async () => {
    // Generate a deeply nested JSON string exceeding the MAX_DEPTH of 32
    const depth = 40;
    let deepJson = '{"a":1}';
    ...
    const request = new Request("https://hsb-boden.de/api/lead", {
      ...
      headers: {
        "Content-Type": "application/json",
        "Content-Length": String(new TextEncoder().encode(deepJson).length),
      },
      body: deepJson,
    });

    const response = await onRequestPost({ request, env: testEnv } as any);
  ...
  it("returns 400 on invalid payload without leaking internals", async () => {
    const res = await onRequestPost(
      makeContext(makeRequest({ ...validBody, email: "not-an-email" })),
    );
  ...
  it("silently rejects a filled honeypot without calling the webhook", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValue(Response.json({ ok: true }));
    const res = await onRequestPost(
      makeContext(makeRequest({ ...validBody, honeypot: "i-am-a-bot" })),
    );
    expect(res.status).toBe(400);
    expect(fetchMock).not.toHaveBeenCalled();
  });
  ...
  it("rejects a foreign Origin", async () => {
    const res = await onRequestPost(
      makeContext(makeRequest(validBody, { origin: "https://evil.example" })),
    );
  ...
  it("accepts a same-project Cloudflare Pages preview Origin", async () => {
    ...
    const res = await onRequestPost(
      makeContext(makeRequest(validBody, { origin })),
    );
    ...
    const res = await onRequestPost(
      makeContext(
        makeRequest(validBody, {
          origin:
            "https://feature-auth-cutover.hsb-boden.pages.dev.evil.example",
        }),
      ),
    );
  ...
  it("rejects a payload larger than 16 KB", async () => {
    const res = await onRequestPost(
      makeContext(makeRequest({ ...validBody, message: "x".repeat(20_000) })),
    );
  ...
  it("does not expose authenticated webhook configuration or lead data on upstream failure", async () => {
    ...
    expect(res.status).toBe(502);
    expect(observableOutput).not.toContain(
      "test-only-auth-token-with-32-characters",
    );
  ...
  it("rate-limits after 5 requests from the same IP within 10 minutes", async () => {
    ...
    for (let i = 0; i < 5; i++) {
      const res = await onRequestPost(
        makeContext(
          makeRequest({ ...validBody, email: `lead${i}@example.com` }, { ip }),
        ),
      );
      expect(res.status).toBe(200);
    }
    const res6 = await onRequestPost(
      makeContext(
        makeRequest({ ...validBody, email: "lead6@example.com" }, { ip }),
      ),
    );
  ...
  it("rate-limits after 2 requests from the same email within 30 minutes regardless of IP", async () => {
    ...
    const res1 = await onRequestPost(
      makeContext(makeRequest({ ...validBody, email }, { ip: "203.0.113.10" })),
    );
    const res2 = await onRequestPost(
      makeContext(makeRequest({ ...validBody, email }, { ip: "203.0.113.11" })),
    );
    const res3 = await onRequestPost(
      makeContext(makeRequest({ ...validBody, email }, { ip: "203.0.113.12" })),
    );
...
describe("GET/PUT/DELETE /api/lead", () => {
  it("rejects GET with 405", async () => {
    const res = await onRequestGet(
      makeContext(makeRequest(undefined, { method: "GET" })),
    );
...
describe("OPTIONS /api/lead", () => {
  it("answers CORS preflight for the own origin", async () => {
    const res = await onRequestOptions(
      makeContext(makeRequest(undefined, { method: "OPTIONS" })),
    );
