...
describe("json-ld generation", () => {
  ...
  it("builds article json-ld for tech articles", () => {
    const graph = buildArticleJsonLd({
      headline: "Epoxidharz vs PU-Beton: Welcher Bodenbelag ist der richtige?",
      description: "Ein umfassender Vergleich der beiden wichtigsten Industriebodensysteme.",
      path: "/ratgeber/epoxidharz-vs-pu-beton/",
    });

    expect(() => JSON.stringify(graph)).not.toThrow();
    expect(graph["@type"]).toBe("TechArticle");
    expect(graph.headline).toBe("Epoxidharz vs PU-Beton: Welcher Bodenbelag ist der richtige?");
    expect(graph.description).toBe("Ein umfassender Vergleich der beiden wichtigsten Industriebodensysteme.");
    ...
    expect(graph.mainEntityOfPage).toContain("/ratgeber/epoxidharz-vs-pu-beton/");
  });
});
